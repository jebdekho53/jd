...
echo ""
echo "=== API health ==="
if curl -sf "${API_URL%/api/v1}/health" >/dev/null 2>&1 || curl -sf "$API_URL/health" >/dev/null 2>&1; then
  pass "API reachable"
else
...
if [[ -n "${ADMIN_BEARER_TOKEN:-}" ]]; then
  HC=$(curl -sS -X POST "$API_URL/admin/logistics/health-check" \
...
if [[ -n "${SHADOWFAX_WEBHOOK_SECRET:-}" ]]; then
  ...
  WH=$(curl -sS -w "\n%{http_code}" -X POST "$API_URL/webhooks/shadowfax" \
...
if [[ -n "${SMOKE_ORDER_ID:-}" ]] && [[ -n "${BUYER_BEARER_TOKEN:-}" ]]; then
  TRACK=$(curl -sS "$API_URL/buyer/orders/$SMOKE_ORDER_ID/tracking" \
