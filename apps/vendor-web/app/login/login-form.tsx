...
/**
 * `set-password` exists because a vendor who was already a JebDekho user (buyer,
 * say) keeps their existing account at approval, and approval deliberately does
 * NOT copy the password chosen on the public application form onto a pre-existing
 * account (the form is unauthenticated — anyone could enter someone else's
 * number). Proving ownership with an OTP is what makes setting the password safe.
 */
...
const BUTTON =
  'w-full rounded-lg bg-violet-500 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-violet-400 disabled:opacity-50';
...
export function LoginForm() {
  ...
  const [error, setError] = useState<string | null>(
    params.get('error') === 'not_a_vendor' ? 'That account is not a vendor partner.' : null,
  );
  ...
  return (
    <div className="mt-8 rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
      ...
      {notice && (
        <p className="mb-3 rounded-lg bg-violet-500/10 px-3 py-2 text-sm text-violet-300">
          {notice}
        </p>
      )}
