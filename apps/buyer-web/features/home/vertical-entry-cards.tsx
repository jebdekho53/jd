 ...
 * of one mixed feed. Self-contained (gradient + icon, no image assets).
 */
  ...
  return (
    <section aria-label="Shop by category" className={className}>
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
        {CARDS.map(({ label, sub, href, icon: Icon, className: cardClass, iconClassName }) => (
          <Link
            key={label}
            href={href}
            className={cn(
          ...
          >
            <span className={cn('grid h-11 w-11 place-items-center rounded-2xl', iconClassName)}>
