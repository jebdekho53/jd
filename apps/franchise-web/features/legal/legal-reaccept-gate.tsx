...
/**
 * Proactive prompt: when the API reports the franchise partner owes acceptance
 * of a legal document, this blocks the dashboard with an accept modal.
 *
 * Fail-OPEN by design. The real enforcement is server-side (the payout gate
 * already requires a signed agreement, and guarded endpoints reject with a 403
 * `LEGAL_ACCEPTANCE_REQUIRED`). So this only blocks on a *definitive* pending
 * list — loading, a network error, a 401, or a malformed response fall through
 * to the app, and the backend remains the backstop.
 */
      ...
      <div className="w-full max-w-lg rounded-xl bg-white shadow-2xl">
        <div className="border-b border-slate-200 px-6 py-4">
          ...
          <p className="mt-1 text-xs text-slate-500">
            {reacceptance
              ? 'We have updated the terms that govern your franchise partnership. Please review and accept the latest version to continue.'
        ...
        <div className="border-t border-slate-200 px-6 py-4">
          ...
          <button
            ...
            onClick={onAccept}
            className="w-full rounded-lg bg-emerald-600 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-50"
          >
