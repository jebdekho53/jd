...
exports.AdminAiProductUsageService = void 0;
const common_1 = require("@nestjs/common");
const client_1 = require("@prisma/client");
const prisma_service_1 = require("../../database/prisma.service");
const merchant_ai_wallet_service_1 = require("../product/merchant-ai-wallet.service");
let AdminAiProductUsageService = class AdminAiProductUsageService {
    constructor(prisma, aiWallet) {
        this.prisma = prisma;
    ...
    buildWhere(filters) {
        ...
        if (filters.merchantProfileId)
            where.merchantProfileId = filters.merchantProfileId;
        if (filters.storeId)
            where.storeId = filters.storeId;
        if (filters.lowConfidence)
            where.confidence = { lt: 0.55 };
        if (filters.charged)
            where.chargedAt = { not: null };
        if (filters.failed)
            where.status = client_1.AIProductAnalysisStatus.FAILED;
        return where;
    }
    async getStats(filters = {}) {
        const where = this.buildWhere(filters);
        const [totalAnalyses, confirmedProducts, failedAnalyses, refundCount, revenueAgg, refundAgg, merchantGroups,] = await Promise.all([
            this.prisma.aIProductAnalysis.count({ where }),
            this.prisma.aIProductAnalysis.count({
                where: { ...where, status: client_1.AIProductAnalysisStatus.CONFIRMED },
            }),
            this.prisma.aIProductAnalysis.count({
                where: { ...where, status: client_1.AIProductAnalysisStatus.FAILED },
            }),
    ...
    async exportCsv(filters = {}) {
        ...
        const header = 'id,merchant,store,confidence,status,chargeAmountPaise,chargedAt,productName,errorMessage,createdAt';
...
exports.AdminAiProductUsageService = AdminAiProductUsageService;
exports.AdminAiProductUsageService = AdminAiProductUsageService = __decorate([
...
//# sourceMappingURL=admin-ai-product-usage.service.js.map