...
let OpenAiVisionClient = OpenAiVisionClient_1 = class OpenAiVisionClient {
    ...
    async generateProductImage(prompt) {
        this.assertConfigured();
        const model = this.configService.get('OPENAI_IMAGE_MODEL', 'gpt-image-1');
        const size = this.configService.get('OPENAI_IMAGE_SIZE', '1024x1024');
        const apiKey = this.configService.get('OPENAI_API_KEY', '');
        let response;
        try {
            response = await axios_1.default.post('https://api.openai.com/v1/images/generations', {
                model,
        ...
        if (typeof url === 'string' && url.length > 0) {
            try {
                const img = await axios_1.default.get(url, {
    ...
    async editProductImage(image, prompt) {
        this.assertConfigured();
        const model = this.configService.get('OPENAI_IMAGE_MODEL', 'gpt-image-1');
        const size = this.configService.get('OPENAI_IMAGE_SIZE', '1024x1024');
        const apiKey = this.configService.get('OPENAI_API_KEY', '');
        ...
        let response;
        try {
            response = await axios_1.default.post('https://api.openai.com/v1/images/edits', form, {
        ...
        const url = response.data?.data?.[0]?.url;
        if (typeof url === 'string' && url.length > 0) {
            const img = await axios_1.default.get(url, { responseType: 'arraybuffer', timeout: 60_000 });
            return Buffer.from(img.data);
        }
        throw new common_1.ServiceUnavailableException({
    ...
    async postVisionRequest(payload, apiKey, timeout) {
        try {
            return await axios_1.default.post('https://api.openai.com/v1/chat/completions', payload, {
                headers: {
                    Authorization: `Bearer ${apiKey}`,
                    'Content-Type': 'application/json',
                },
    ...
    describeOpenAiFailure(error) {
        ...
        const upstreamMessage = typeof error.response?.data?.error?.message === 'string'
    ...
    stripPlaceholder(value) {
        return OpenAiVisionClient_1.PLACEHOLDER_VALUES.has(value.trim().toLowerCase()) ? '' : value;
    }
...
OpenAiVisionClient.PLACEHOLDER_VALUES = new Set([
