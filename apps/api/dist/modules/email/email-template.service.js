...
let EmailTemplateService = class EmailTemplateService {
    layout(title, bodyHtml, preheader) {
        const { primary, secondary, muted, background, white } = email_constants_1.EMAIL_COLORS;
        return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8" />
  ...
  <div class="wrap">
    ...
    orderConfirmation(data) {
        ...
        const rows = data.items
            .map((i) => `<tr><td>${this.escape(i.name)}</td><td>${i.qty}</td><td>${this.escape(i.price)}</td></tr>`)
            .join('');
        const body = `
    ...
    orderDelivered(orderNumber, reviewUrl) {
    ...
    adminPasswordReset(resetUrl, expiresMinutes) {
    ...
    adminNewDeviceLogin(name, ipAddress) {
    ...
    merchantApproved(storeName, dashboardUrl) {
    ...
    merchantRejected(storeName, reason, dashboardUrl) {
