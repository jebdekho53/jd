...
/** Nearest-neighbor route optimization for pickup then drop stops */
export function optimizeRoute(stops: RouteStop[], start: { lat: number; lng: number }): RouteOptimizationResult {
  if (stops.length === 0) {
  ...
  const visitNearest = (pool: RouteStop[]) => {
    while (pool.length > 0) {
      let bestIdx = 0;
