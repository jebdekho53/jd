...
import { AIProductAnalysisStatus, MerchantAiCreditTransactionType } from '@prisma/client';
...
const mockUpload = { uploadImage: jest.fn() };
const mockVision = {
...
describe('ProductAiService', () => {
  ...
  it('returns availability when OpenAI not configured', () => {
  ...
  it('does not charge on analyze', async () => {
    ...
    mockUpload.uploadImage.mockResolvedValue({ url: 'https://cdn/img.jpg' });
  ...
  it('throws when OpenAI key missing on analyze', async () => {
    ...
    expect(mockUpload.uploadImage).not.toHaveBeenCalled();
  });
  ...
  it('charges on confirm', async () => {
    ...
    expect(mockBilling.chargeForProductCreation).toHaveBeenCalledWith('mp-1', 's-1', 'a-1');
  ...
  it('lists billing history', async () => {
    mockPrisma.merchantAiCreditTransaction.findMany.mockResolvedValue([
      {
        analysisId: 'a-1',
        amountPaise: 150,
        status: 'SUCCESS',
    ...
    mockPrisma.merchantAiCreditTransaction.findFirst.mockResolvedValue(null);
