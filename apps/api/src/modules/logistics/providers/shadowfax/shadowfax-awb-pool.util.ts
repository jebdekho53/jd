const AWB_PATTERN = /^([A-Z]+)(\d+)([A-Z]*)$/;
const AWB_RANGE_LIMIT = 100_000;

function expandAwbRange(token: string, allowedPrefixes: ReadonlySet<string>): string[] | null {
  ...
  if (!allowedPrefixes.has(start[1])) return null;
...
export function parseShadowfaxAwbPool(
  raw: string | undefined | null,
  allowedPrefixes: string[] = ['SF'],
): string[] {
  const seen = new Set<string>();
  const awbs: string[] = [];
  const prefixes = new Set(allowedPrefixes.map((prefix) => prefix.toUpperCase()));
  const tokens = (raw ?? '')
    .split(/[\s,]+/)
  ...
  for (const token of tokens) {
    const prefix = token.match(/^([A-Z]+)/)?.[1];
    const expanded = expandAwbRange(token, prefixes) ?? (prefix && prefixes.has(prefix) && /^[A-Z]+[A-Z0-9]+$/.test(token) ? [token] : []);
