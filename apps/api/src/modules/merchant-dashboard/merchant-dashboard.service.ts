...
export class MerchantDashboardService {
  ...
  async resolveStoreContext(userId: string, storeId?: string): Promise<StoreContext> {
    ...
    if (storeId) {
      ...
      if (!store) {
        throw new ForbiddenException('Store does not belong to merchant');
      }
  ...
  async getOverview(userId: string, query: MerchantDashboardStoreQueryDto) {
    try {
      ...
      const [
        ...
        this.prisma.$queryRaw<{ day: Date; orders: bigint; revenue: Prisma.Decimal }[]>`
      ...
      return {
        ...
        deliveredToday: countByStatus([OrderStatus.DELIVERED, OrderStatus.COMPLETED]),
        cancelledOrders: countByStatus(CANCELLED_STATUSES),
        avgOrderValue,
        customerRating: Math.round((storeRating._avg.ratingAvg ?? 0) * 10) / 10,
        ratingCount: storeRating._sum.ratingCount ?? 0,
        growth: {
          ordersPct: pctChange(todayOrders, yesterdayOrders),
      ...
      this.logger.error(
        `getOverview failed (userId=${userId}, storeId=${query.storeId ?? 'all'})`,
  ...
  async getOrders(userId: string, query: MerchantDashboardOrdersQueryDto) {
    try {
      const { storeIds } = await this.resolveStoreContext(userId, query.storeId);
      ...
      if (storeIds.length === 0) {
        return {
          orders: [],
          tabs: Object.fromEntries(Object.keys(ORDER_TAB_STATUSES).map((tab) => [tab, 0])),
  ...
  async getInventory(userId: string, query: MerchantDashboardStoreQueryDto) {
    const empty = {
      summary: {
        totalProducts: 0,
        activeProducts: 0,
    ...
    try {
      ...
      const [products, lowStock, topSellers] = await Promise.all([
        ...
        this.prisma.$queryRaw<
          { product_id: string; product_name: string; variant_id: string; quantity: number; threshold: number }[]
        >`
      ...
      for (const p of products) {
        ...
        if (p.isActive && defaultVariant?.isActive !== false) activeProducts += 1;
        else hiddenProducts += 1;
  ...
  async getRiders(userId: string, query: MerchantDashboardStoreQueryDto) {
    ...
    try {
      ...
      const onlineCount = riders.filter(
        (r) =>
          r.status === RiderStatus.ONLINE ||
          r.status === RiderStatus.ON_DELIVERY ||
  ...
  async getAnalytics(userId: string, query: MerchantDashboardAnalyticsQueryDto) {
    ...
    try {
      ...
      if (storeIds.length === 0) {
        ...
        return emptyMerchantAnalytics(period);
      }
      ...
      const [dailyStats, categoryStats, hourlyStats] = await Promise.all([
        this.prisma.$queryRaw<{ day: Date; orders: bigint; revenue: Prisma.Decimal }[]>`
          ...
          WHERE store_id = ANY(${storeIds}::text[])
            ...
            AND ${sqlOrderStatusIn(Prisma.sql`status`, REVENUE_STATUSES)}
        ...
        this.prisma.$queryRaw<{ category_name: string; revenue: Prisma.Decimal; units: bigint }[]>`
          ...
          LIMIT 15
        `,
        this.prisma.$queryRaw<{ hour: number; orders: bigint }[]>`
      ...
      const sorted = [...itemPerformance].sort(
      ...
      const [ordersToday, ordersThisWeek, ordersThisMonth, cancelledInPeriod, acceptedInPeriod, paidInPeriod] =
        await Promise.all([
          this.prisma.order.count({
            where: { storeId: { in: storeIds }, createdAt: { gte: todayStart }, status: { notIn: CANCELLED_STATUSES } },
          }),
          this.prisma.order.count({
            where: { storeId: { in: storeIds }, createdAt: { gte: weekStart }, status: { notIn: CANCELLED_STATUSES } },
          }),
          this.prisma.order.count({
            where: { storeId: { in: storeIds }, createdAt: { gte: monthStart }, status: { notIn: CANCELLED_STATUSES } },
          }),
          this.prisma.order.count({
            where: { storeId: { in: storeIds }, createdAt: { gte: since }, status: { in: CANCELLED_STATUSES } },
          }),
          this.prisma.order.count({
            where: {
              storeId: { in: storeIds },
              createdAt: { gte: since },
              status: { in: [OrderStatus.MERCHANT_ACCEPTED, OrderStatus.PREPARING, OrderStatus.PACKING, OrderStatus.READY_FOR_PICKUP, OrderStatus.RIDER_ASSIGNED, OrderStatus.PICKED_UP, OrderStatus.OUT_FOR_DELIVERY, OrderStatus.DELIVERED, OrderStatus.COMPLETED] },
            },
          }),
          this.prisma.order.count({
            where: { storeId: { in: storeIds }, createdAt: { gte: since }, status: OrderStatus.PAID },
          }),
        ]);
      ...
      if (error instanceof ForbiddenException) throw error;
      this.logger.error(
        `getAnalytics failed (userId=${userId}, storeId=${query.storeId ?? 'all'}, period=${period})`,
        error instanceof Error ? error.stack : String(error),
      );
      return emptyMerchantAnalytics(period);
    }
  ...
  async getCustomers(userId: string, query: MerchantDashboardStoreQueryDto) {
    const empty = {
      totalCustomers: 0,
      ...
      topCustomers: [],
      recentReviews: [],
      recentComplaints: [] as { id: string; message: string; createdAt: string }[],
    };
  ...
  async getCompliance(userId: string, query: MerchantDashboardStoreQueryDto) {
    const empty = {
      stores: [],
    ...
    try {
      ...
      return {
        stores: stores.map((s) => ({
          ...
          fssaiProvided: docTypes.has(StoreDocumentType.FSSAI_LICENSE),
        ...
        categoryRequests: {
          approved: categoryRequests.find((c) => c.status === StoreCategoryRequestStatus.APPROVED)?._count.id ?? 0,
          pending: categoryRequests.find((c) => c.status === StoreCategoryRequestStatus.PENDING)?._count.id ?? 0,
          underReview: categoryRequests.find((c) => c.status === StoreCategoryRequestStatus.UNDER_REVIEW)?._count.id ?? 0,
          rejected: categoryRequests.find((c) => c.status === StoreCategoryRequestStatus.REJECTED)?._count.id ?? 0,
          documentsRequired:
            categoryRequests.find((c) => c.status === StoreCategoryRequestStatus.DOCUMENTS_REQUIRED)?._count.id ?? 0,
        },
        alerts: stores
          .filter((s) =>
            (
  ...
  async getNotifications(userId: string, query: MerchantDashboardStoreQueryDto) {
    const empty = {
      recentOrders: [],
      newReviews: [],
      inventoryAlerts: 0,
