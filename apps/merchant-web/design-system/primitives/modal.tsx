  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      <button
        ...
        aria-label="Close"
      />
      <div className={cn('relative z-10 w-full rounded-xl bg-white shadow-xl', W[size])}>
        <div className="flex items-start justify-between border-b border-slate-100 px-5 py-4">
          ...
          {dismissible && (
            <button type="button" onClick={onClose} className="ml-4 rounded-md p-1 text-slate-400 hover:bg-slate-100 hover:text-slate-600">
              <X className="h-4 w-4" />
            </button>
          )}
        </div>
