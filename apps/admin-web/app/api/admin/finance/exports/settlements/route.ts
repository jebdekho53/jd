...
export async function GET(req: NextRequest) {
  try {
    ...
    const path = `${getApiBaseUrl()}/admin/finance/exports/settlements${qs ? `?${qs}` : ''}`;
    ...
    const disposition = res.headers.get('content-disposition') ?? 'attachment; filename="settlements.csv"';
