 ...
 * so typing "wha" surfaces "WhatsApp Inbox" above anything that merely lists
 * "whatsapp" as a keyword.
 */
...
export function CommandPalette() {
  ...
  const results = useMemo(() => {
    ...
    return NAV.map((item) => ({ item, rank: score(item, q) }))
      .filter(({ rank }) => rank > 0)
  ...
  // Cmd/Ctrl+K toggles from anywhere except while typing in another field.
  useEffect(() => {
  ...
  return (
    <>
      <button
        ...
        className="flex w-full items-center gap-2 rounded-lg border border-slate-700 bg-slate-800/60 px-3 py-2 text-sm text-slate-400 transition-colors hover:border-slate-600 hover:text-slate-200"
      >
        <Search className="h-4 w-4 flex-shrink-0" aria-hidden />
        <span className="flex-1 text-left">Search…</span>
        <kbd className="rounded border border-slate-700 px-1.5 py-0.5 text-[10px] font-medium text-slate-500">
          ⌘K
        </kbd>
      </button>
          ...
          >
            <div className="flex items-center gap-2 border-b border-slate-100 px-4">
              ...
              <input
                ...
                placeholder="Jump to a page — try “whatsapp”, “orders”, “payouts”"
                    ...
                    <li key={item.href}>
                      <button
                        ...
                        className={cn(
                          ...
                          index === activeIndex ? 'bg-slate-100 text-slate-900' : 'text-slate-600',
                        )}
                      >
